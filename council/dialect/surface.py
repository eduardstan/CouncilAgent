...
def render_move(move: Move, *, anonymize: bool = False) -> str:
    ...
    match move:
        ...
        case Retract(own=own, why=ret_why):
            suffix = f" ({ret_why.surface})" if ret_why else ""
            return f"{agent} retracts {own}{suffix}."

        case Question(target=tgt, query=q):
            qtext = q.surface or "(no question)"
            return f"{agent} asks {tgt}: {qtext}"

        case Clarify(target=tgt, restated=rest):
            rtext = rest.surface or "(no clarification)"
        ...
        case Abstain(why=abs_why):
            w = f": {abs_why}" if abs_why else ""
